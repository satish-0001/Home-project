...
let device;
frappe.ui.form.on('Start Campaign', {
	 refresh: function(frm) {
	    if(frm.doc.status == "Online"){
	        frm.add_custom_button(__('Break Start'), function() {
				frm.set_value('status','Break-Start')
				frm.save()
            });
	    }
	    if(frm.doc.status == "Break-Start"){
	        frm.add_custom_button(__('Break End'), function() {
				frm.set_value('status','Break-End')
				frm.save()
            });
	    }
	    if(frm.doc.status == "Break-End" || frm.doc.status == "Offline"){
	        frm.add_custom_button(__('Online'), function() {
				frm.trigger("get_calls");
            });
	    }
	    if(frm.doc.status == "Online"){
...
//	    if(frm.doc.status == "Online"){
//	        frm.add_custom_button(__('Start Calling'), function() {

//	            $.each(frm.doc.start_campaign_call_list, function(i, m) {
//	                frm.set_value('dialer',m.call);
//	                let to_numbers;
//	                if (Array.isArray(frm.doc.dialer)) {
//                        to_numbers = frm.doc.dialer;
//                    } else {
//                        to_numbers = frm.doc.dialer.split('\n');
//                    }
//	                let outgoing_call_popup;
//	                outgoing_call_popup = new OutgoingCallPopup(device, to_numbers);
//			        outgoing_call_popup.show();
//                    if($(".btn btn-secondary btn-xs prev-page").click()){
...
//	    }
	 ...
	 onload:function(frm){
//        frm.get_field("start_campaign_call_list").grid.cannot_add_rows = true;
	 },

	 get_calls:function(frm){
	    frm.call({
				doc: frm.doc,
				freeze: true,
				method: "get_call",
				...
				callback: (r) => {
					if(!r.exc && r.message){
					    ...
					    frm.set_value('status','Online');
						frm.refresh_fields("start_campaign_call_list",)
						frm.save();
					}
				}
			});
